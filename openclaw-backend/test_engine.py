...
class TestAgent:
    ...
    async def generate_test_content(self, file_path: str, code: str) -> str:
        ...
        try:
            test_code = await self.call_llm_fn(prompt)
    ...
    async def cycle(self, file_path: str, code: str, broadcast_fn):
        """Orchestrates the generate-then-run cycle."""
        # 1. Generate Tests
        test_content = await self.generate_test_content(file_path, code)
        if not test_content:
            return
        ...
        if test_path:
            ...
            # 3. Run Tests
            result = await self.run_tests(file_path)
            
            # 4. Broadcast
            if broadcast_fn:
                await broadcast_fn(file_path, result)

# Global instance placehoder
