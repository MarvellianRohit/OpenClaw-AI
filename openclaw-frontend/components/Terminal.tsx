...
import LeakWarningModal from "./LeakWarningModal";
...
export default function Terminal({ isOpen, onClose, onLineReceived }: TerminalProps) {
    ...
    useEffect(() => {
        if (isOpen && !socket) {
            ...
            ws.onmessage = (event) => {
                try {
                    // Check if message is a JSON object with type 'memory_leak'
                    const data = JSON.parse(event.data);
                    if (data.type === "memory_leak") {
                        setLeakData(data.data);
                        setIsLeakModalOpen(true);
                        return; // Don't show JSON in terminal output
                    }
                } catch (e) {
                    // Not JSON or other error, treat as raw text
                }
